...
- (void)setModel:(TZAssetModel*)model {
    ...
    int32_t imageRequestID = [[TZImageManager manager] getPhotoWithAsset:model.asset
                                                              photoWidth:self.tz_width
...
- (void)setType:(TZAssetCellType)type {
    ...
    if (type == TZAssetCellTypePhoto || type == TZAssetCellTypeLivePhoto || (type == TZAssetCellTypePhotoGif && !self.allowPickingGif) ||
        self.allowPickingMultipleVideo) {
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[TZImageManager manager] requestImageDataForAsset:_model.asset
        completion:^(NSData* imageData, NSString* dataUTI, UIImageOrientation orientation, NSDictionary* info) {
    ...
    if (_model.type == TZAssetCellTypeVideo) {
        [[TZImageManager manager] getVideoWithAsset:_model.asset
...
- (void)setModel:(TZAlbumModel*)model {
    ...
    NSAttributedString* countString = [[NSAttributedString alloc]
        initWithString:[NSString stringWithFormat:@"  (%zd)", model.count]
    ...
    [[TZImageManager manager] getPostImageWithAlbumModel:model
                                              completion:^(UIImage* postImage) {
