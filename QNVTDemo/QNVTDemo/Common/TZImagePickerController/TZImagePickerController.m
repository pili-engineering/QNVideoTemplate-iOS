    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            NSString* tipText = [NSString
                stringWithFormat:[NSBundle tz_localizedStringForKey:@"Allow %@ to access your album in \"Settings -> Privacy -> Photos\""], appName];
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[TZImageManager manager] authorizationStatusAuthorized]) {
        ...
        if ([albumPickerVc isKindOfClass:[TZAlbumPickerController class]]) {
            dispatch_async(dispatch_get_main_queue(), ^{ [albumPickerVc loadPhotos]; });
        }
    }
}
...
- (void)setCircleCropRadius:(NSInteger)circleCropRadius {
    ...
    self.cropRect = CGRectMake(
        self.view.tz_width / 2 - circleCropRadius, self.view.tz_height / 2 - _circleCropRadius, _circleCropRadius * 2, _circleCropRadius * 2);
}
...
@property(nonatomic, strong) MCPageView* segmentView;
@property(nonatomic, strong) TZPhotoPickerController* photoPickerVC;
@property(nonatomic, strong) TZPhotoPickerController* videoPickerVC;
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem* cancelItem = [[UIBarButtonItem alloc] initWithTitle:imagePickerVc.cancelBtnTitleStr
    ...
    _photoPickerVC = [[TZPhotoPickerController alloc] init];
    _videoPickerVC = [[TZPhotoPickerController alloc] init];

    [self addChildViewController:self.photoPickerVC];
    [self addChildViewController:self.videoPickerVC];

    _segmentView = [[MCPageView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT)
                                              titles:@[ @"图片", @"视频" ]
                                         controllers:@[ _photoPickerVC, _videoPickerVC ]];
    _segmentView.translatesAutoresizingMaskIntoConstraints = NO;
    _segmentView.titileBackgroudColor = QNVTCOLOR(232323);
    _segmentView.defaultTitleColor = UIColor.whiteColor;
    _segmentView.selectTitleColor = QNVTCOLOR(1f66ff);
    [self.view addSubview:_segmentView];

    [[_segmentView.topAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.topAnchor] setActive:YES];
    [[_segmentView.leftAnchor constraintEqualToAnchor:self.view.leftAnchor] setActive:YES];
    [[_segmentView.rightAnchor constraintEqualToAnchor:self.view.rightAnchor] setActive:YES];
    [[_segmentView.bottomAnchor constraintEqualToAnchor:self.view.bottomAnchor] setActive:YES];
                                     ...
                                     needFetchAssets:YES
                                          completion:^(TZAlbumModel* model) {
                                              dispatch_async(dispatch_get_main_queue(), ^{
                                                  self.photoPickerVC.columnNumber = self.columnNumber;
        ...
        [[TZImageManager manager] getCameraRollAlbum:YES
                                   allowPickingImage:NO
                                     needFetchAssets:YES
                                          completion:^(TZAlbumModel* model) {
                                              dispatch_async(dispatch_get_main_queue(), ^{
                                                  self.videoPickerVC.columnNumber = self.columnNumber;
                                                  self.videoPickerVC.model = model;
...
+ (BOOL)tz_isRightToLeftLayout {
    if (@available(iOS 9.0, *)) {
        if ([UIView userInterfaceLayoutDirectionForSemanticContentAttribute:UISemanticContentAttributeUnspecified] ==
