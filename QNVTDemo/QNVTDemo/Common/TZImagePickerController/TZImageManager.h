...
/// Get full Image 获取原图
/// 如下两个方法completion一般会调多次，一般会先返回缩略图，再返回原图(详见方法内部使用的系统API的说明)，如果info[PHImageResultIsDegradedKey]
/// 为 YES，则表明当前返回的是缩略图，否则是原图。
...
- (void)getVideoOutputPathWithAsset:(PHAsset*)asset
    ...
    __attribute__((deprecated("Use -getVideoOutputPathWithAsset:failure:success:")));
