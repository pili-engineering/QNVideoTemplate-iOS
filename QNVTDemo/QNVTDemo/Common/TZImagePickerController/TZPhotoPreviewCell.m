...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(photoPreviewCollectionViewDidScroll)
                                                     name:@"photoPreviewCollectionViewDidScroll"
                                                   object:nil];
    }
    return self;
}
...
- (void)setAsset:(PHAsset*)asset {
    ...
    _asset = asset;
    self.imageRequestID = [[TZImageManager manager] getPhotoWithAsset:asset
        completion:^(UIImage* photo, NSDictionary* info, BOOL isDegraded) {
...
- (void)refreshScrollViewContentSize {
    if (_allowCrop) {
        ...
        BOOL small = self.scrollView.tz_width < self.scrollView.contentSize.width;
        CGFloat contentWidthAdd =
            small ? self.scrollView.tz_width - CGRectGetMaxX(_cropRect) : (self.scrollView.contentSize.width - _cropRect.size.width) / 2.0;
        CGFloat contentHeightAdd = small ? (MIN(_imageContainerView.tz_height, self.tz_height) - self.cropRect.size.height) / 2
                                         : (self.scrollView.contentSize.height - _cropRect.size.height) / 2.0;
        ;
...
- (void)configMoviePlayer {
    ...
    if (self.model && self.model.asset) {
        [[TZImageManager manager] getPhotoWithAsset:self.model.asset
                                         completion:^(UIImage* photo, NSDictionary* info, BOOL isDegraded) {
                                             BOOL iCloudSyncFailed = !photo && [TZCommonTools isICloudSyncError:info[PHImageErrorKey]];
                                             self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
                                             self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
        ...
        [[TZImageManager manager] getVideoWithAsset:self.model.asset
