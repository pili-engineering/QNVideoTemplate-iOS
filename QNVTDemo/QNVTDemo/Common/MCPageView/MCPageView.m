...
- (void)layoutSubviews {
    ...
    UICollectionViewFlowLayout* flowLayout = (UICollectionViewFlowLayout*)self.contentCollection.collectionViewLayout;
...
- (void)animationItem:(BOOL)isleft percent:(CGFloat)percent index:(NSInteger)index {
    ...
    self.netxColor = [UIColor colorWithRed:self.defaultR + (self.selectedR - self.defaultR) * percent
                                     green:self.defaultG + (self.selectedG - self.defaultG) * percent
                                      blue:self.defaultB + (self.selectedB - self.defaultB) * percent
                                     alpha:self.defaultA + (self.selectedA - self.defaultA) * percent];

    UIColor* lastColor = [UIColor colorWithRed:self.selectedR - (self.selectedR - self.defaultR) * percent
                                         green:self.selectedG - (self.selectedG - self.defaultG) * percent
                                          blue:self.selectedB - (self.selectedB - self.defaultB) * percent
                                         alpha:self.selectedA - (self.selectedA - self.defaultA) * percent];
...
- (void)getColorRGB:(UIColor*)color
...
- (void)selectIndex:(NSInteger)index {
    ...
    self.isClick = YES;
    [self changeItemStatus:index];
    [self.contentCollection scrollToItemAtIndexPath:[NSIndexPath indexPathForRow:labs(index) inSection:0]
...
- (void)setTitileBackgroudColor:(UIColor*)titileBackgroudColor {
    _titileBackgroudColor = titileBackgroudColor;
    self.titleScroll.backgroundColor = titileBackgroudColor;
}
...
- (UICollectionView*)contentCollection {
    if (!_contentCollection) {
        ...
        _contentCollection =
            [[UICollectionView alloc] initWithFrame:CGRectMake(0, self.titleScroll.bottom, kwidth, self.height - self.titleScroll.bottom)
...
- (UIScrollView*)titleScroll {
    if (!_titleScroll) {
        ...
        _titleScroll.backgroundColor = _titileBackgroudColor;
        ...
        [_contentTitles enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL* _Nonnull stop) {
            @autoreleasepool {
                MCItem* item = [[MCItem alloc]
                    initWithFrame:CGRectMake(idx * weakSelf.titleButtonWidth, 0, weakSelf.titleButtonWidth, weakSelf.titleScrollHeight)];
